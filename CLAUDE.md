...
- `npx astro sync` — regenerates `astro:env` and content collection types; run after changing env schema or on a fresh checkout before lint/build

Pre-commit hooks: husky + lint-staged runs `eslint --fix` on `*.{ts,tsx,astro}` and `prettier --write` on `*.{json,css,md}`.

**Tests**: Playwright is configured. Run with `npx playwright test` (no `npm test` script). Tests live in `tests/auth.spec.ts`. Add new test files alongside it.
...
- **Tailwind v4**: wired via `@tailwindcss/vite` Vite plugin — no `tailwind.config.ts` exists by default. Do not create one unless explicitly adding customization.
...
- **Supabase security**: Create one policy per operation (SELECT, INSERT, UPDATE, DELETE) per role. Never use FOR ALL or USING (true) on tables with user data.
...
Two GitHub Actions workflows run on every push and PR to `main`/`master`:

- `.github/workflows/ci.yml` — lint + build. Requires `SUPABASE_URL` and `SUPABASE_KEY` repository secrets.
- `.github/workflows/playwright.yml` — Playwright e2e tests. Uploads HTML report as an artifact on failure.
